'''
In this file we are calculating Region of Interest(ROI) by receiving points from mouse event and transform perspective so that
...
We have used YOLOv4 as our deep learning model and it is used to detect humans in frame and by calculating bottom center point of bounding boxes around humans,we transform those points to bird eye view. And then calculates risk factor by calculating distance between
...
# horizontal line and point 5 and 7 should form vertical line. Horizontal and vertical scale will be different.
...
def calculate_social_distancing(vid_path, net, output_dir, output_vid, ln1):
    ...
    ####################################################################################
        ...
        # YOLO v4 model for detection of pedestrians
        boxes1 = yolo.detect_pedestrians(frame, net,ln1,H,W)
        if len(boxes1) == 0:
            count = count + 1
            continue
...
if __name__== "__main__":
    ...
    parser.add_argument('-v', '--video_path', action='store', dest='video_path', default='./data/demo.mp4' ,help='Path for input video')
    ...
    # loading Yolov4 weights...
    
    weightsPath = model_path + "yolov4.weights"
    configPath = model_path + "yolov4.cfg"
